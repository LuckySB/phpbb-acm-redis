...
class acm extends acm_memory
{
	...
	function acm()
	{
		...
		$this->redis_ro = new Redis();
		$this->redis->connect(PHPBB_ACM_REDIS_HOST, PHPBB_ACM_REDIS_PORT);
		$this->redis_ro->connect('localhost', PHPBB_ACM_REDIS_PORT);
		...
		if (defined('PHPBB_ACM_REDIS_PASSWORD'))
		{
			...
			if (!$this->redis_ro->auth(PHPBB_ACM_REDIS_PASSWORD))
			{
				global $acm_type;

				trigger_error("Incorrect password for the ACM module $acm_type. slave", E_USER_ERROR);
			}
		}
		...
		$this->redis_ro->setOption(Redis::OPT_SERIALIZER, Redis::SERIALIZER_PHP);
		$this->redis_ro->setOption(Redis::OPT_PREFIX, $this->key_prefix);
		...
		if (defined('PHPBB_ACM_REDIS_DB'))
		{
			...
			if (!$this->redis_ro->select(PHPBB_ACM_REDIS_DB))
			{
				global $acm_type;

				trigger_error("Incorrect database for the ACM module $acm_type. slave", E_USER_ERROR);
			}
		}
	}
